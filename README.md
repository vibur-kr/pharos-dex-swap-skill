...
A Pharos Agent Center Skill that enables AI agents to swap tokens on Pharos through FaroSwap Uniswap V3 pools via natural language commands. **Single-command execution** — the agent runs one `helpers.py` command that handles pool discovery, quoting, wrapping, approval and swap. Auto-executes when safety checks pass.

## Features

- **Single-Command Swap** — One `python3 helpers.py swap` call does everything: pool lookup, quote, wrap, approve, execute. No multi-step process.
...
- **Native Token Handling** — Auto-wraps PROS to WPROS before swapping
...
6. **Wrap** native PROS → WPROS if needed
7. **Approve** exact swap amount via DODO Approve Proxy
8. **Encode** `mixSwap()` calldata
9. **Execute** swap on FaroSwap Router with 0.5% slippage protection
10. **Return** JSON result with tx hash, amounts, rate, explorer link
