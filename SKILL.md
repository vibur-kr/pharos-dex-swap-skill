...
```bash
# Find the project root — look for .env or use current directory
PROJECT_ROOT=$(pwd)
...
```bash
set -a && source .env && set +a && [ -n "$PRIVATE_KEY" ] && echo "PRIVATE_KEY: set" || echo "PRIVATE_KEY: not set"
```
...
```bash
# Replace /absolute/path/to with the actual project root path from Step 0.1
set -a && source /absolute/path/to/.env && set +a && ...rest of command...
```

**Why absolute path:** Relative `source .env` fails if the agent changes directory (e.g. `cd` into a subfolder). The agent MUST use the full absolute path like `source /home/user/project/.env` or `source C:/Users/user/project/.env` every single time.
...
**IMPORTANT — Discovery speed:** `cast logs` over thousands of blocks is SLOW. The agent MUST use the fast discovery methods below, starting with Method 1 (explorer API). Only fall back to `cast logs` as a last resort with a very narrow range (500 blocks max).
...
If cached pools exist, skip to Step 2.5 to verify they are still active (pools can be drained between sessions).

If no cache or cache is empty, proceed with discovery using the methods below **in order**. Stop at the first method that finds pools.

### Method 1: Block Explorer API (fastest — use first)

Use the Pharos block explorer API to find recent transactions involving the token. Extract pool/router addresses from those transactions.

**Testnet explorer:**
```bash
# Get recent transactions for a token
curl -s "https://pharos-testnet.socialscan.io/api?module=account&action=txlist&address=$TOKEN_ADDRESS&sort=desc&page=1&offset=20" | jq -r '.result[]?.to' | sort -u
```

**Mainnet explorer:**
```bash
# Get recent transactions for a token
curl -s "https://www.pharosscan.xyz/api?module=account&action=txlist&address=$TOKEN_ADDRESS&sort=desc&page=1&offset=20" | jq -r '.result[]?.to' | sort -u
```

**Socialscan API (testnet):**
```bash
# Alternative: get token transfer events
curl -s "https://pharos-testnet.socialscan.io/api?module=account&action=tokentx&address=$TOKEN_ADDRESS&sort=desc&page=1&offset=20" | jq -r '.result[]?.to' | sort -u
```

From the returned addresses, filter out EOAs (addresses with no code). For each contract address, verify it is a DODO pool (see Method 3 Step 2).

### Method 2: DODO Factory Direct Call (fast — no scanning)

If a DODOZoo factory address is known or cached, call it directly:
...
If the factory returns a non-zero address → that's a pool. Verify it (Step 2.5).

To find the factory address: check the block explorer API (Method 1) for contract addresses that many pools share as a creator, or check `state.json` cache.

### Method 3: Narrow cast logs Scan (fallback — slow)

Only use this if Methods 1 and 2 failed. Use the **smallest possible range**:

```bash
BLOCK=$(cast block-number --rpc-url $PHAROS_RPC_URL)
# ONLY 500 blocks — enough to find active pools, fast to scan
FROM_BLOCK=$((BLOCK - 500))

cast logs --address $TOKEN_IN --from-block $FROM_BLOCK --to-block $BLOCK \
  "Transfer(address,address,uint256)" --rpc-url $PHAROS_RPC_URL
```

If this times out or returns nothing:
1. Try scanning `tokenOut` instead of `tokenIn`
2. Reduce to 200 blocks
3. If still nothing → report "No pools found" to the user

Parse the output to extract unique `from` and `to` addresses. For each unique address:

1. Check if it is a contract:
...
2. Try calling DODO pool functions:
...
### Step 2.4: Filter Pools by Token Pair
...
### Step 2.5: Verify Pools and Check Reserves
