...
def encode_mix_swap_calldata(
    ...
    Returns full calldata hex WITH 0x prefix and method ID.
    """
    method_id = 'ff84aafa'

    extra_data = encode_bytes_array(['0' * 63 + 'c0'])
    user_data = '0' * 128
    ...
    head = ''
    head += encode_address(from_token)
    head += encode_address(to_token)
    head += encode_uint256(amount)
    head += encode_uint256(min_return)
    head += encode_uint256(expected)
    head += encode_uint256(off_mix_adapters)
    head += encode_uint256(off_asset_ids)
    head += encode_uint256(off_path_ids)
    head += encode_uint256(direction)
    head += encode_uint256(off_extra_data)
    head += encode_uint256(off_user_data)
    head += encode_uint256(deadline)
...
NATIVE = '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeEEeEe'
...
def cmd_swap(args):
    ...
    if is_native:
        bal = int(_first(_get_native_balance(rpc, wallet)))
        # Leave 0.01 native for gas
        gas_reserve = int(0.01 * (10 ** from_decimals))
    ...
    amount_out_human = amount_out_human = amount_out_raw / (10 ** to_decimals)

    # Handle native token: wrap + approve
    actual_from = from_info['address']
    if is_native:
        wrapped_sym = from_info['wrapped']
        wrapped_addr = net['tokens'][wrapped_sym]['address']
        _log(f'Wrapping {amount_float} {from_sym} -> {wrapped_sym}...')
        _send(rpc, private_key, wrapped_addr, 'deposit()', value=str(amount_raw))
        _log(f'Approving {wrapped_sym}...')
        _send(rpc, private_key, wrapped_addr, 'approve(address,uint256)',
              net['contracts']['approve_proxy'], str(amount_raw))
        actual_from = wrapped_addr
    else:
    ...
    result = _send(rpc, private_key, net['contracts']['router'], calldata)
