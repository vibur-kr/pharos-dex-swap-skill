...
description: Pharos DEX token swap via FaroSwap Uniswap V3 pools. Single-command swap/balance/send — helpers.py handles pool discovery, V3 quoting, approval and execution automatically. Auto-executes swaps when safety checks pass.
version: 2.0.0
frameworks:
  - openclaw
  - claude-code
...
Swap tokens on Pharos through FaroSwap Uniswap V3 pools. One command does everything — pool discovery, quoting, approval, and execution. The user says "swap 10 PROS to USDC" and gets the result. Only stops for problems (price impact, insufficient liquidity, low balance).
...
1. **Native tokens** — `PHRS`, `PROS`, `ETH` (as native) → the native gas token. Address is `0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee`. Sent as `value` with the swap transaction — no wrapping needed. The FaroSwap Router handles wrapping automatically.
...
- **Exact-Amount Approval** — Approve tokens for the exact swap amount only via Inner Proxy. NEVER approve unlimited spending.
...
- **Native Token Handling** — Native PROS/PHRS sent as `value` with the transaction. No wrapping needed — the FaroSwap Router handles it automatically.
...
| DODO Approve Proxy | `0x2aFc65f51B8afd1dB9618643F89f0b135EaAeEAa` | Token approvals (outer proxy) |
| Inner Proxy | `0xBF105f4ffBD3825f5433d074008B9A76237d849c` | Does actual transferFrom — **approve this for ERC20** |
...
- `0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee` — represents native PROS/PHRS in mixSwap calls
...
| Parameter | Value |
|-----------|-------|
| fromToken | input token address (`0xeeee...` for native) |
| toToken | output token address (`0xeeee...` for native) |
...
6. For native PROS: sends as `value` with tx (no wrap needed). For ERC20: approves Inner Proxy
7. Encodes `mixSwap` calldata with pool pair info in extraData
8. Executes the swap via FaroSwap Router
...
3. **Safety**: Blocks if price impact > 3%, liquidity = 0, or balance insufficient (0.1 PROS gas reserve).
4. **Native token**: Sends PROS as `value` with tx (no wrapping). Router handles wrapping internally.
5. **ERC20**: Approves exact amount to Inner Proxy (`0xBF105f...`), which is the contract that actually calls `transferFrom`.
6. **Execution**: Encodes `mixSwap` calldata with pool pair info in extraData, sends via FaroSwap Router with 0.5% slippage.
7. **All steps** run in a single process — one command, one terminal approval, zero back-and-forth.
