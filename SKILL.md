...
description: Token swap on Pharos DEX — auto-discovers DODO-based pools (FaroSwap, Zenith, custom), checks liquidity and price impact, protects against bad trades with slippage guard and exact-amount-only approvals, executes swap via cast, returns tx hash and explorer link.
version: 1.0.0
frameworks:
  - openclaw
  - claude-code
...
Swap tokens on Pharos through FaroSwap and other DODO-based DEX pools. The skill auto-discovers all available pools for a token pair, checks liquidity, warns about price impact, and executes swaps safely with exact-amount approvals and slippage protection.
...
| Quote reverts | Pool may be paused or broken. Skip it, try other pools. |
...
2. **"Trade 0.5 ETH for USDC on Pharos testnet"** — Native token wrapping + swap
